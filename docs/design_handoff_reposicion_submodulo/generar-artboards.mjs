...
// reportes/impresion.tsx (Membrete, FilaKv, thImpresa/tdImpresa, PieDePagina) y wizard/Step1Scan.
// Estudios reales; cantidades, lotes y nombres de medicamentos INVENTADOS.
//
...
const vista = (cab, inner) => `<div style="width: ${VISTA}px;">${cab}<div style="padding: 16px 26px 26px;">${inner}</div></div>`
...
/** Desplegable cerrado (fieldInput + chevron): valores fijos, sin texto libre. */
const desplegable = (valor, placeholder = false, w = 280) => `<div style="width: ${w}px; height: 44px; border: 1px solid ${T.line2}; border-radius: 10px; background: ${T.white}; display: flex; align-items: center; gap: 8px; padding: 0 12px 0 14px; font-size: 14px; color: ${placeholder ? T.faint : T.ink};"><span style="flex: 1; min-width: 0;">${valor}</span>${ic('chevronDown', 16, T.muted)}</div>`
...
const avisoItem = (texto, tono = 'info') => {
  ...
  return `<div style="display: flex; gap: 8px; align-items: flex-start; padding: 5px 0; font-size: 12.5px; line-height: 1.45; color: ${tono === 'warn' ? T.deepWarn : T.ink};">
  ${ic(tono === 'warn' ? 'alert' : 'info', 14, c, 1.9, ' margin-top: 2px;')}<span style="flex: 1; min-width: 0;">${texto}</span>
</div>`
}
/** Estado del pedido como pastilla (texto siempre, el color acompaña). */
const pildora = (texto, tono = 'neutro') => {
  const t = {
    neutro: `color: ${T.inkSoft}; background: ${T.surface}; border: 1px solid ${T.line};`,
    warn: `color: ${T.deepWarn}; background: rgba(176, 130, 63, 0.14); border: 1px solid transparent;`,
    good: `color: ${T.deepGood}; background: rgba(92, 138, 90, 0.14); border: 1px solid transparent;`,
    muted: `color: ${T.muted}; background: ${T.surface}; border: 1px solid ${T.line}; text-decoration: line-through;`,
  }[tono]
  return `<span style="font-size: 11.5px; font-weight: 600; padding: 3px 10px; border-radius: 999px; white-space: nowrap; ${t}">${texto}</span>`
}
...
/** La tarjeta de Pacientes, con la parte de abajo cambiada por lo que falta comprar y el pedido. */
const tarjetaEstudio = ({ codigo, nombre, principal, detalle = '', pedido = '', link = true, estado = 'Activo', colorEstado = T.good }) => `<div style="background: ${T.white}; border: 1px solid ${T.line}; border-radius: 16px; padding: 18px 20px; display: flex; flex-direction: column; gap: 8px;">
  ...
  <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px; min-height: 30px;">
    <div style="display: flex; align-items: baseline; gap: 7px; min-width: 0;">${principal}</div>
...
const lineaPedido = (texto, tono = 'normal') => `<div style="display: flex; align-items: center; gap: 7px; font-size: 12.5px; color: ${tono === 'mudo' ? T.muted : tono === 'warn' ? T.deepWarn : T.ink};">${ic('truck', 14, tono === 'mudo' ? T.faint : tono === 'warn' ? T.deepWarn : PH, 1.8)}<span>${texto}</span></div>`

const franjaCorte = `<div style="display: flex; align-items: center; gap: 10px; margin: 0 0 14px;">
  ${ic('calendar', 16, PH)}
  <span style="font-size: 14px; font-weight: 600; color: ${T.ink};">Corte el 28/09 · faltan 12 días</span>
  <span style="font-size: 12.5px; color: ${T.inkSoft};">Período 29/08 → 28/09. Lo que pase después del corte va al período siguiente.</span>
</div>`

const grilla = (tarjetas) => `<div style="display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px;">${tarjetas.join('')}</div>`

const tarjetasRecorrido = [
  tarjetaEstudio({ codigo: '222714', nombre: 'ENDURA', principal: paraComprar(11), detalle: '2 medicamentos · 1 sin cargar', pedido: lineaPedido('Sin pedido para el período que viene', 'mudo') }),
  tarjetaEstudio({ codigo: 'ACT18301', nombre: 'ACT', principal: paraComprar(6), detalle: '1 medicamento', pedido: lineaPedido('Sin pedido para el período que viene', 'mudo') }),
  tarjetaEstudio({ codigo: 'CKJX839D12302', nombre: 'Victorion', principal: sinMedicacion, link: false }),
  tarjetaEstudio({ codigo: 'LTS17231', nombre: 'Extensión de ACT18301', principal: cubierto, pedido: lineaPedido('Pedido Nº 13 · recibido en parte') }),
]

const main = page(vista(
  cabecera({ acciones: accion('Día de corte: 28', 'calendar') }),
  franjaCorte + grilla(tarjetasRecorrido),
))

// ═══════════════════════════ 2 · El estudio (libro + boleta) ═══════════════════════════

const COLS = 'grid-template-columns: minmax(0, 1fr) 84px 84px 84px 84px 190px 44px;'
const COLS_CERRADO = 'grid-template-columns: minmax(0, 1fr) 96px 96px 96px 96px;'
const cabTabla = (ultima = 'Comprar') => `<div style="display: grid; ${ultima ? COLS : COLS_CERRADO} border-bottom: 1px solid ${T.line2};">
  ${th('Medicamento')}${th('Había', 'right')}${th('Entró', 'right')}${th('Salió', 'right')}${th(ultima ? 'Hay' : 'Quedó', 'right')}${ultima ? `${th(ultima, 'right')}<div></div>` : ''}
</div>`
const nombreMed = (nombre, presentacion, avisoAmbar = '') => `<div style="padding: 13px 16px; min-width: 0;">
  <div style="display: flex; align-items: center; gap: 7px; font-size: 14px; color: ${T.ink};"><span style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${nombre}</span>${avisoAmbar ? ic('alert', 14, T.deepWarn, 1.9) : ''}</div>
...
const filaLibro = ({ nombre, presentacion, habia, entro, salio, hay, comprar, abierto = false, ultimo = false, aviso = false }) => `<div style="display: grid; ${COLS} align-items: center; ${ultimo && !abierto ? '' : `border-bottom: 1px solid ${abierto ? T.line : T.line};`}${abierto ? ` background: ${T.surface};` : ''}">
  ${nombreMed(nombre, presentacion, aviso)}
  ${celda(num(habia))}${celda(num(entro))}${celda(num(salio))}${celda(num(hay))}
  <div style="padding: 13px 16px; display: flex; align-items: baseline; justify-content: flex-end; gap: 6px;">${comprar}</div>
...
const comprarN = (n) => `${grande(n)}${envasesTxt(n)}`
...
const boletaSalbutral = boleta([
  lineaBoleta('Hacen falta para el próximo período', '8 pacientes, 1 envase por mes', '', '8'),
  lineaBoleta('Van a quedar en el estante al corte', 'hay 5, y 4 pacientes todavía no retiraron', '−', '1'),
], '7', `<div style="display: flex; gap: 18px; margin-top: 12px;">${linkMudo('Cambiar cómo se repone')}</div>`)

const resumenPeriodo = (texto, numero, detalle) => `<div style="display: flex; align-items: center; gap: 20px; padding: 16px 20px; margin: 0 0 14px; background: ${T.white}; border: 1px solid ${T.line}; border-radius: 16px;">
  <div style="flex: 1; min-width: 0;">
    <div style="font-size: 12.5px; color: ${T.inkSoft};">${texto}</div>
    <div style="display: flex; align-items: baseline; gap: 8px; margin-top: 2px;">${numero}</div>
  </div>
...
const navPeriodo = (texto, sub, siguiente = false) => `<div style="display: flex; align-items: center; gap: 8px; margin: 0 0 14px 50px;">
  ${flecha('chevronLeft')}
  <span class="mono" style="font-size: 14px; font-weight: 600; color: ${T.ink}; padding: 0 4px;">${texto}</span>
  ${flecha('chevronRight', siguiente)}
  <span style="font-size: 12.5px; color: ${T.inkSoft}; margin-left: 6px;">${sub}</span>
</div>`

const seccionPedidos = (filas) => `<div style="margin-top: 22px;">
...
const filaPedido = ({ numero, emitido, para, estado, tono, ultimo = false }) => `<div style="display: grid; grid-template-columns: 90px 170px minmax(0, 1fr) auto 70px; align-items: center; gap: 12px; padding: 12px 16px; ${ultimo ? '' : `border-bottom: 1px solid ${T.line};`}">
  <span class="mono" style="font-family: ${DISPLAY}; font-weight: 700; font-size: 15px; color: ${T.ink};">Nº ${numero}</span>
  <span style="font-size: 12.5px; color: ${T.inkSoft};">Emitido el <span class="mono">${emitido}</span></span>
  <span style="font-size: 12.5px; color: ${T.inkSoft};">Para el período <span class="mono">${para}</span></span>
  ${pildora(estado, tono)}
  <span style="text-align: right;">${linkMudo('Ver')}</span>
</div>`

const estudio = page(vista(
  cabecera({ migas: ['222714'] }),
  encabezadoEstudio(accion('Armar pedido', 'cart', true))
  + navPeriodo('29/08 → 28/09', 'Período en curso · corte en 12 días')
  + resumenPeriodo(
    'Para el período que viene (29/09 → 28/10)',
    `${grande(11)}${envasesTxt(11)}`,
    '2 medicamentos para comprar · <span style="color: ' + T.deepWarn + '; font-weight: 600;">1 sin cargar</span>',
  )
  + card(
    cabTabla()
    ...
    + filaLibro({ nombre: 'Seretide 250/50', presentacion: 'Aerosol', habia: 5, entro: 15, salio: 12, hay: 8, comprar: comprarN(4), aviso: true })
    + filaLibro({ nombre: 'Tiotropio 18 mcg', presentacion: 'Cápsulas', habia: 4, entro: 0, salio: 1, hay: 3, comprar: noSeCompra, ultimo: true }),
  )
  + seccionPedidos([
    filaPedido({ numero: 12, emitido: '28/08', para: '29/08 → 28/09', estado: 'recibido', tono: 'good', ultimo: true }),
  ]),
))
...
const armarPedido = page(modal(
  ...
  `<div style="display: grid; ${COLS_PED} gap: 8px; border-bottom: 1px solid ${T.line2};">
    <div style="padding: 0 0 8px; font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: ${T.inkSoft};">Medicamento</div>
    <div style="padding: 0 0 8px; font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: ${T.inkSoft}; text-align: right;">Calculado</div>
    <div style="padding: 0 0 8px; font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: ${T.inkSoft}; text-align: right;">Pedir</div>
  </div>
  ...
  <div style="font-size: 12.5px; color: ${T.inkSoft}; margin-top: 6px; line-height: 1.45;">Se guarda con número y queda para recibirlo en Recepción. Un renglón en 0 no va al pedido.</div>
  ...
  560,
  'Para el período 29/09 → 28/10. Tiotropio no aparece: no se compra.',
))

// ═══════════════════════════ 4 · La hoja (impresion.tsx: Membrete, FilaKv, th/tdImpresa, Pie) ═══════════════════════════

// Las tres últimas columnas se llenan A MANO en la farmacia: renglones altos y aire a la izquierda.
const thI = (t, align = 'left', w = '', izq = 0) => `<th style="text-align: ${align}; font-size: 9px; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; padding: 6px 8px 6px ${izq}px; border-bottom: 1px solid #000;${w ? ` width: ${w};` : ''}">${t}</th>`
const tdI = (t, align = 'left', izq = 0) => `<td style="padding: 14px 8px 14px ${izq}px; border-bottom: 1px solid #999; font-size: 10.5px; vertical-align: top; text-align: ${align};">${t}</td>`
const kv = (k, v) => `<tr><td style="padding: 5px 0; border-bottom: 1px solid #999; font-size: 11.5px; width: 46%;">${k}</td><td style="padding: 5px 0; border-bottom: 1px solid #999; font-size: 11.5px; font-weight: 700;">${v}</td></tr>`
const hoja = page(`<div style="width: 794px; height: 1123px; background: #FFFFFF; color: #000; padding: 56px 60px; font-family: 'Inter', system-ui, sans-serif; display: flex; flex-direction: column;">
  ...
  <table style="width: 100%; border-collapse: collapse;">
    ...
    <tbody>
      <tr>${tdI('<b>Salbutral 100 mcg</b>')}${tdI('Aerosol')}${tdI('<b class="mono">7</b>', 'right')}${tdI('', 'left', 18)}${tdI('', 'left', 12)}${tdI('', 'left', 12)}</tr>
      <tr>${tdI('<b>Seretide 250/50</b>')}${tdI('Aerosol')}${tdI('<b class="mono">6</b>', 'right')}${tdI('', 'left', 18)}${tdI('', 'left', 12)}${tdI('', 'left', 12)}</tr>
    </tbody>
  </table>
  <div style="font-size: 10.5px; margin-top: 8px;">Total pedido: <b class="mono">13 envases</b> de 2 medicamentos.</div>
  ...
  <div style="margin-top: 22px; padding: 9px 12px; border: 1px solid #000; font-size: 11px; font-weight: 700;">Devolver esta hoja junto con la medicación. En Recepción se recibe por el número del pedido.</div>
...
const COLS_DET = 'grid-template-columns: minmax(0, 1fr) 70px 80px 60px 150px;'
...
const filaDet = ({ nombre, presentacion, pedido, recibido, falta, accionDet = '', ultimo = false, extra = '' }) => `<div style="${ultimo ? '' : `border-bottom: 1px solid ${T.line};`}">
  <div style="display: grid; ${COLS_DET} gap: 8px; align-items: center; padding: 11px 0;">
    <div style="min-width: 0;"><div style="font-size: 14px; color: ${T.ink};">${nombre}</div><div style="font-size: 11.5px; color: ${T.inkSoft}; margin-top: 2px;">${presentacion}</div></div>
    <div style="text-align: right;">${num(pedido)}</div>
    <div style="text-align: right;">${num(recibido)}</div>
    <div style="text-align: right;">${falta === 0 ? ic('check', 15, T.deepGood, 2, ' margin-left: auto;') : num(falta, T.deepWarn)}</div>
    <div style="text-align: right;">${accionDet}</div>
  </div>
  ${extra}
</div>`
const recepcionesDelPedido = `<div style="margin-top: 16px;">
  <div style="font-size: 10.5px; font-weight: 700; letter-spacing: 0.11em; text-transform: uppercase; color: ${T.inkSoft}; margin-bottom: 6px;">Recepciones de este pedido</div>
  <div style="display: flex; align-items: center; gap: 10px; font-size: 12.5px; color: ${T.ink}; padding: 6px 0;">${ic('clipboardCheck', 15, PH)}<span class="mono" style="font-weight: 600;">Nº 1051</span><span style="color: ${T.inkSoft};">02/10 · verificada por Agustín Bazzani · 12 envases</span></div>
</div>`
const pedidoDetalle = page(modal(
  'Pedido Nº 14',
  `<div style="display: flex; align-items: center; gap: 10px; margin: 0 0 14px;">${pildora('Recibido en parte', 'warn')}<span style="font-size: 12.5px; color: ${T.inkSoft};">Falta 1 envase</span></div>
  ${cabDet}
  ${filaDet({ nombre: 'Salbutral 100 mcg', presentacion: 'Aerosol', pedido: 7, recibido: 7, falta: 0 })}
  ${filaDet({ nombre: 'Seretide 250/50', presentacion: 'Aerosol', pedido: 6, recibido: 5, falta: 1, accionDet: linkMudo('No va a llegar'), ultimo: true })}
  ${recepcionesDelPedido}
  <div style="font-size: 12.5px; color: ${T.inkSoft}; margin-top: 14px; line-height: 1.45;">Ya tiene una recepción, así que no se puede anular. Lo que falta cuenta como «ya pedido» hasta que llegue o se cierre.</div>
  <div style="display: flex; gap: 10px; margin-top: 18px;">${btnOutline(`${ic('printer', 16, T.ink)}Reimprimir`)}<div style="flex: 1;"></div>${btnPrimary('Listo')}</div>`,
  580,
  '222714 · ENDURA · para el período 29/09 → 28/10 · emitido el 28/09 por Lautaro Molina',
))
...
const filaRecibir = ({ numero, codigo, nombre, meta, estado, tono, aviso = '', ultimo = false }) => `<div style="display: grid; grid-template-columns: 64px minmax(0, 1fr) auto; align-items: center; gap: 14px; padding: 13px 0; ${ultimo ? '' : `border-bottom: 1px solid ${T.line};`}">
  <span class="mono" style="font-family: ${DISPLAY}; font-weight: 700; font-size: 16px; color: ${T.ink};">Nº ${numero}</span>
  <div style="min-width: 0;">
    <div style="display: flex; align-items: center; gap: 8px;"><span class="mono" style="font-size: 14px; font-weight: 700; color: ${PH};">${codigo}</span><span style="font-size: 13.5px; color: ${T.ink};">${nombre}</span>${pildora(estado, tono)}</div>
    <div style="font-size: 12.5px; color: ${T.inkSoft}; margin-top: 3px;">${meta}</div>
    ${aviso}
  </div>
  ${btnPrimary(`${ic('truck', 15, T.onAccent)}Recibir`, PH, 36)}
</div>`
const recibirPedido = page(modal(
  'Recibir un pedido',
  `${filaRecibir({ numero: 13, codigo: 'LTS17231', nombre: 'Extensión de ACT18301', meta: 'Emitido el 27/08 · falta 1 envase de 1 medicamento', estado: 'recibido en parte', tono: 'warn' })}
  ${filaRecibir({ numero: 14, codigo: '222714', nombre: 'ENDURA', meta: 'Emitido el 28/09 · faltan 13 envases de 2 medicamentos', estado: 'sin recibir', tono: 'neutro' })}
  ${filaRecibir({ numero: 15, codigo: 'ACT18301', nombre: 'ACT', meta: 'Emitido el 28/09 · faltan 6 envases de 1 medicamento', estado: 'sin recibir', tono: 'neutro', aviso: `<div style="margin-top: 4px;">${avisoItem('Ya tiene la recepción Nº 1043 sin verificar: fijate antes de recibirlo de nuevo.', 'warn')}</div>`, ultimo: true })}`,
  600,
  'Los pedidos con algo por recibir, del más viejo al más nuevo. Buscá el número que figura en la hoja.',
))

// ═══════════════════════════ 7 · El asistente arranca con el pedido (Step1Scan) ═══════════════════════════

const stepper = (n) => `<div style="display: inline-flex; align-items: center; gap: 6px;">
...
const filaEscaneo = ({ nombre, codigo, n, meta, metaTono = 'normal', primero = false }) => `<div style="display: flex; align-items: center; gap: 14px; padding: 11px 18px; ${primero ? '' : `border-top: 1px solid ${T.line};`}">
  ...
  <span style="font-size: 12.5px; color: ${metaTono === 'warn' ? T.deepWarn : T.inkSoft}; white-space: nowrap;">${meta}</span>
  ${stepper(n)}
</div>`
const asistente = page(`<div style="width: 900px; padding: 28px 40px;">
  <div style="display: flex; flex-direction: column; gap: 14px; max-width: 820px; margin: 0 auto;">
    <div style="display: flex; align-items: center; gap: 12px; padding: 13px 16px; background: ${T.white}; border: 1px solid ${T.line}; border-radius: 12px;">
      ...
      <div style="flex: 1; min-width: 0;">
        ...
        <div style="font-size: 12.5px; color: ${T.inkSoft}; margin-top: 2px;">Vienen puestos los 2 medicamentos con lo que falta. Si llegó menos, bajá la cantidad: lo que falta queda en el pedido.</div>
      </div>
    </div>
    ${card(`${filaEscaneo({ nombre: 'Salbutral 100 mcg', codigo: '7791234567890', n: 7, meta: 'pedidos 7', primero: true })}
      ${filaEscaneo({ nombre: 'Seretide 250/50', codigo: '7790987654321', n: 5, meta: 'pedidos 6 · llegan 5' })}
      ${filaEscaneo({ nombre: 'Budesonida 200 mcg', codigo: '7795555555555', n: 2, meta: 'No estaba en el pedido', metaTono: 'warn' })}`)}
    <p style="margin: 0; font-size: 12.5px; color: ${T.inkSoft}; text-align: center;">El resto del asistente sigue igual: lotes, vencimientos y confirmar. En el resumen final se ve lo pedido contra lo recibido.</p>
  </div>
</div>`)
...
const estadosTarjeta = page(vista(
  cabecera(),
  grilla([
    tarjetaEstudio({ codigo: '222714', nombre: 'ENDURA', principal: paraComprar(11), detalle: '2 medicamentos · 1 sin cargar', pedido: lineaPedido('Pedido Nº 14 · sin recibir') }),
    tarjetaEstudio({ codigo: 'LTS17231', nombre: 'Extensión de ACT18301', principal: cubierto, pedido: lineaPedido('Pedido Nº 13 · recibido · faltó 1') }),
    tarjetaEstudio({ codigo: 'ACT18301', nombre: 'ACT', principal: faltaCargar, detalle: '0 de 3 cargados', pedido: lineaPedido('Sin pedido para el período que viene', 'mudo') }),
    tarjetaEstudio({ codigo: 'CKJX839D12302', nombre: 'Victorion', principal: sinMedicacion, link: false }),
  ]),
))
...
const diaCorte = page(modal(
  'Día de corte',
  `${label('Día del mes')}${inputNum('28', '')}
  <div style="font-size: 12.5px; color: ${T.inkSoft}; margin-top: 12px; line-height: 1.5;">Próximos cortes: <span class="mono">28/09 · 28/10 · 28/11</span>. Si un mes no tiene ese día, corta el último.</div>
...
const periodoAnterior = page(vista(
  cabecera({ migas: ['222714'] }),
  encabezadoEstudio('')
  + navPeriodo('29/07 → 28/08', 'Período cerrado', true)
  + `<div style="display: flex; align-items: center; gap: 9px; margin: 0 0 14px; font-size: 13px; color: ${T.inkSoft};">${ic('info', 15, T.inkSoft, 1.9)}<span>Un período cerrado muestra lo que entró y salió. La compra se arma en el período en curso.</span>${linkMudo('Ir al período en curso')}</div>`
  + card(
    cabTabla(null)
  ...
  + seccionPedidos([
    filaPedido({ numero: 9, emitido: '28/07', para: '29/07 → 28/08', estado: 'recibido', tono: 'good', ultimo: true }),
  ]).replace('<div style="margin-top: 22px;">', '<div style="margin-top: 22px; max-width: 820px;">'),
))
...
const boletas = page(`<div style="width: 1040px; padding: 28px;">
  <div style="display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px;">
    ${tarjetaBoleta('Salbutral 100 mcg · a demanda', `<span style="display: inline-flex; align-items: baseline; gap: 6px;">${comprarN(3)}</span>`, boleta([
    ...
    ${tarjetaBoleta('Seretide 250/50 · con un pedido en camino', enCamino(6), boleta([
      lineaBoleta('Hacen falta para el próximo período', '12 pacientes, 1 envase por mes', '', '12'),
      lineaBoleta('Van a quedar en el estante al corte', 'hay 8 y ya retiraron todos', '−', '8'),
      lineaBoleta('Ya pedido, sin recibir', 'Pedido Nº 14 del 15/09', '−', '6'),
    ], '0', '', 'alcanza'))}
    ${tarjetaBoleta('Salbutral 100 mcg · el estante no alcanza', `<span style="display: inline-flex; align-items: baseline; gap: 6px;">${comprarN(9)}</span>`, boleta([
      lineaBoleta('Hacen falta para el próximo período', '8 pacientes, 1 envase por mes', '', '8'),
      lineaBoleta('Faltan para terminar este período', '4 pacientes todavía no retiraron y en el estante no alcanza', '+', '1'),
    ], '9'))}
    ${tarjetaBoleta('Seretide 250/50 · todo vencido', `<span style="display: inline-flex; align-items: baseline; gap: 6px;">${comprarN(12)}</span>`, boleta([
      lineaBoleta('Hacen falta para el próximo período', '12 pacientes, 1 envase por mes', '', '12'),
      lineaBoleta('Van a quedar en el estante al corte', 'hay 0 y ya retiraron todos, sin contar 10 vencidos', '−', '0'),
    ], '12', `<div style="margin-top: 10px;">${avisoItem('Hay 10 envases vencidos en el estante.', 'warn')}</div>`))}
  </div>
</div>`)
...
const noVaALlegar = page(modal(
  ...
  ${filaDet({
    ...
    extra: `<div style="margin: 0 0 12px; padding: 14px; border-radius: 11px; background: ${T.surface}; border: 1px solid ${T.line};">
      <div style="font-size: 13px; color: ${T.ink}; margin-bottom: 10px;">El envase que falta deja de contar como pedido y vuelve a la compra.</div>
      ${label('Por qué no va a llegar')}
      ${desplegable('La farmacia no lo tiene', false, 300)}
...
const anular = page(modal(
  ...
  `<div style="font-size: 13px; color: ${T.ink}; line-height: 1.5; margin: 0 0 14px;">Todavía no se recibió nada. Queda en la lista como anulado y deja de contar como pedido.</div>

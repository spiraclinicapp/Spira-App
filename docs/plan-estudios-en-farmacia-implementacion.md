...
- **Migración `0139`**, nombre `0138_estudios_en_farmacia.sql`. Verificado libre: la última aplicada
  en prod es la 0138 y nadie reservó la 0139 en docs.
...
| `supabase/README.md` | **Modificar.** La fila del índice para la 0139 (CI lo vigila con `scripts/check-migraciones.mjs`). |
...
## Tarea 1 · Migración 0139, parte A: el modelo y las siete funciones

**Archivos:**
- Crear: `supabase/migrations/0138_estudios_en_farmacia.sql`
- Banco de pruebas (descartable, **no se commitea**): `<scratchpad>/pglite-0139/`
...
```
Expected: `marcadores: 16 PAR ok` (8 funciones × 2). Si dice IMPAR, hay un `$` suelto en un
...
el `\$` antes de que node lo vea y el regex cuenta cualquier cosa — la primera corrida de esto dio
...
Run:
```bash
export REPO="$(git rev-parse --show-toplevel)" && export BANCO="$(dirname "$(mktemp -u)")/pglite-0139" && mkdir -p "$BANCO" && cd "$BANCO" && npm init -y >/dev/null && npm i @electric-sql/pglite --no-save && echo "BANCO=$BANCO"
```
...
git commit -m "feat(db): 0139 parte A - modelo y funciones de alcance de Farmacia"
```

---

## Tarea 2 · Migración 0139, parte B: la auditoría
...
```bash
git add supabase/migrations/0138_estudios_en_farmacia.sql
git commit -m "feat(db): 0139 parte B - auditoria del alcance de Farmacia"
```

---

## Tarea 3 · Migración 0139, parte C: los dos RPC
...
```bash
git add supabase/migrations/0138_estudios_en_farmacia.sql
git commit -m "feat(db): 0139 parte C - los dos RPC de alcance, con compare-and-swap"
```

---

## Tarea 4 · Migración 0139, parte D: el recorte de nueve policies
...
- [ ] **Paso 4: Registrar la 0139 en el índice del README**
...
git commit -m "feat(db): 0139 parte D - recorte de estudios y pacientes en Farmacia"
```
...
```tsx
  /** El interruptor de cada persona del centro (`ve_todos_los_estudios`, 0139). Bajan por prop
...
```
     2. ESTUDIOS — sobre qué PACIENTES (Coordinación) y sobre qué ESTUDIOS (Farmacia). La otra
        ...
        `pharma_protocol_access`, 0139). Por eso no son dos renglones de un mismo cuadro: un
...
que diga **tres** fuentes, sumando la 0139.
...
El SQL tiene que correr **tal cual**, sin placeholders. Avisarle en el chat que la 0139 está lista,

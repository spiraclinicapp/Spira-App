...
| **G9** | **El encabezado se compone solo; no imita una fila de tabla.** La primera versión alineaba el folio con "Medicamento" y la fecha con "Código", como pedía el handoff. Alineaba al píxel y **hacía ruido igual**, porque son dos sistemas peleando en la misma franja: uno identifica un documento, el otro enumera renglones; alinearlos no los une, los superpone. Ahora es un flex: identidad a la izquierda (Recepción + Recibido juntos, 32px entre ellos), origen a la derecha, y el hueco del medio como separación. Medido: **787px de aire** sobre una card de 1089. Las **cuatro columnas del medio** de la tabla pasan a centradas: los bordes cargan el peso, el medio respira. Pedido del Director. |

...
| **A2** | **Tabla HTML real** (`table-layout: fixed`) + **una sola constante** con los seis porcentajes, consumida por el header en grilla y por los `<col>`. |
...
| **G6** | **Se va el rótulo del folio; la fecha conserva el suyo.** "Nº 1043" a 21px se explica solo. La fecha **no**, porque en la card conviven dos: **RECIBIDO** (cuándo llegó la mercadería) en el encabezado, e **ingresada a stock** en la banda. **Pueden ser días distintos** — el pedido queda apoyado hasta que alguien lo cuenta — y ahí el rótulo deja de ser decoración y pasa a ser desambiguación, que es justo lo que `DESIGN.md` permite: *"las MAYÚSCULAS con tracking son para rótulos puntuales, nunca un eyebrow decorativo"*. Corregido el 2026-08-17 a pedido del Director: la primera versión las sacaba las dos. |

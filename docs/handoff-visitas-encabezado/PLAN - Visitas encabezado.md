...
| 9 | **El nombre y el Nº de sujeto abren la ficha del paciente** (pedido del Director, fuera del handoff) | Reusa el mecanismo del buscador global: `onNavigate(module.key, 'protocolos', { patientId })` → `ProtocolsView` deriva el protocolo y abre la ficha. Se va al **mismo módulo en el que ya estás**, así no hay que evaluar permisos. Estilo `.spira-textlink`: hereda tipografía y color, subraya solo al apuntar o enfocar — el dato tiene que seguir leyéndose como el dato. Desde la ficha del paciente NO se pasa el callback (llevaría a donde ya estás) y el texto queda pelado, sin botón. |

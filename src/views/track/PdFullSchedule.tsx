...
/**
 * Cronograma vertical: todas las visitas del paciente. Por fila: círculo de estado
 * (check si realizada, número si pendiente, actual con anillo + halo), "Visita N",
 * "Semana W#", fecha y pill del estado calculado.
 */
      ...
      {ordered.map((v, k) => {
        ...
        return (
          <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 4px', borderTop: k ? '1px solid var(--spira-line)' : 'none' }}>
            ...
            >
              {done ? <Icon name="check" size={cur ? 15 : 13} color="#fff" stroke={3} /> : idx.get(v.id)}
            </span>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontFamily: 'var(--spira-font-display)', fontWeight: 700, fontSize: 14.5, color: cur ? accent : 'var(--spira-ink)', whiteSpace: 'nowrap' }}>Visita {idx.get(v.id)}</div>
              <div style={{ fontSize: 11.5, color: 'var(--spira-muted)', marginTop: 1 }}>Semana W{weekNumber(v)}</div>
            </div>
            <span className="spira-mono" style={{ fontSize: 12.5, color: 'var(--spira-muted)', minWidth: 56, textAlign: 'right', whiteSpace: 'nowrap' }}>{formatShortAR(v.estimated_date)}</span>

...
import { registerVisit } from '../../data/visits'
...
/**
 * Modal único de "Registrar visita", con comportamiento por etapa:
 *  · Pre-randomización: selector de tipo suelto permitido (firma/screening/firma_screening/vnp/
 *    randomización, filtrado por reglas) → register_visit_event. Registrar la randomización genera
 *    el cronograma.
 *  · Post-randomización: por defecto propone la próxima visita programada (marca su real_date); o se
 *    elige VNP/Retest (visita extra, no consume la programada).
 */
export function RegisterVisitFlow({ enrollmentId, randomizationDate, usedKinds, nextScheduled, nextScheduledLabel, accentSolid, onClose, onDone }: {
  enrollmentId: string
  randomizationDate: string | null
  usedKinds: VisitKind[]
  nextScheduled: TrackVisitRow | null
  ...
  const options: { value: string; label: string }[] = [
    ...(nextScheduled ? [{ value: SCHED, label: `Próxima del cronograma · ${nextScheduledLabel}` }] : []),
  ...
  const isScheduled = choice === SCHED
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true)
    setError(null)
    const res = isScheduled && nextScheduled
      ? await registerVisit(nextScheduled.id, date)
      : await registerVisitEvent(enrollmentId, choice as VisitKind, date, notes.trim() || null)
    setBusy(false)
        ...
        {isRando && (
          <div style={{ display: 'flex', gap: 9, padding: '11px 13px', borderRadius: 11, background: accentSolid + '0E', border: `1px solid ${accentSolid}30` }}>
            ...
            <div style={{ fontSize: 12.5, lineHeight: 1.45, color: 'var(--spira-ink)' }}>
              Al registrar la randomización se genera el cronograma de visitas anclado en esta fecha. Cierra la etapa de screening.
            </div>
          </div>
        )}

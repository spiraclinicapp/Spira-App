...
const paciente = (over: Partial<PatientRow> = {}): PatientRow => ({
  ...
  enrollments: [{
    id: 'enr-default', enrollment_date: '2026-01-01', randomization_date: null, ivrs_code: null, status: 'activo',
...
describe('navDesdePath · el paciente tiene que pertenecer al protocolo del path', () => {
  ...
  const pacienteA = paciente({
    ...
    code: '111',
    enrollments: [{
      id: 'enr-a', enrollment_date: '2026-01-01', randomization_date: null, ivrs_code: null, status: 'activo',
  ...
  const pacienteB = paciente({
    ...
    code: '222',
    enrollments: [{
      id: 'enr-b', enrollment_date: '2026-01-01', randomization_date: null, ivrs_code: null, status: 'activo',
...
describe('resolverFichaDestino', () => {
  ...
  const paciente = (protocolIds: string[]): PatientRow => ({
    ...
    enrollments: protocolIds.map((pid, i) => ({
      id: `enr-${i}`, enrollment_date: '2026-01-01', randomization_date: null, ivrs_code: null, status: 'activo', protocol: proto(pid, `P${i}`),
    })),
  })
  ...
  it('sin enrolamiento visible manda a Todos los pacientes, nunca a una ficha sin contexto', () => {
    const sinProtocolo: PatientRow = { ...paciente([]), enrollments: [
      { id: 'e', enrollment_date: '2026-01-01', randomization_date: null, ivrs_code: null, status: 'activo', protocol: null },
    ] }

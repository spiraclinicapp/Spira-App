// generar.mjs — limpieza de todo lo anterior a la carga de la prueba real (2026-09-15).
//
// Decisión del Director (2026-09-15): lo que había en la app antes de la carga es de la fase alpha, sin
// contacto con usuarios, y es basura. Se borra TODO salvo la configuración y el catálogo de medicamentos,
// incluida la auditoría de eso: «no la puedo dejar como auditable porque no sirve». No se re-discute.
//
// Escribe dos SQL para el editor de Supabase, del mismo molde (sólo cambia v_aplicar):
//   1-simular.sql  Hace la limpieza completa y la deshace al final (raise exception): el «error» ES el informe.
//   2-aplicar.sql  La misma limpieza, sin deshacer. Termina con el conteo de cada tabla.
//
// Uso:  node supabase/scripts/limpieza-prueba-real/generar.mjs
//
// QUÉ QUEDA
//   · Configuración: usuarios, módulos y roles, accesos a estudios, preferencias; los 4 protocolos de la prueba
//     con su cronograma, procedimientos, reportes y plataformas; plantillas.
//   · Catálogo: medicamentos, drogas, laboratorios y sus códigos, y qué medicamentos usa cada estudio.
//   · La carga: las 72 inscripciones (ivrs_code cargado en los 4 estudios), sus 64 personas, sus visitas del
//     cronograma y su medicación de base, EXACTAMENTE como las dejó la carga.
//   · La auditoría desde la carga en adelante.
//
// QUÉ SE VA
//   · THESEUS (EFC18244), CEREN-2 (EFC18419), PROT-A y PROT-B, con todo lo suyo.
//   · Toda inscripción y paciente que no sea de la carga (en ACT18301: los dos sin IVRS).
//   · Todo lo operativo de Farmacia: el stock queda en cero y los comprobantes vuelven a 1.
//   · Todo lo operativo de Coordinación sobre visitas (comentarios, tildes, reportes, alertas archivadas,
//     timeline, checklists, tareas, notas de agenda) y el feedback.
//   · Sobre los 72: visitas sueltas (VNP, retest), sellos de atención, médico, ausencias y notas anteriores.
//   · La configuración de reposición inventada del QA (demora de compra, modo por medicamento).
//   · La auditoría anterior a la carga. Y el borrado mismo no deja rastro: los triggers de usuario (auditoría,
//     guardas, sellos) se apagan durante el bloque y se vuelven a prender al final, en la misma transacción.
//     Los de FK son internos de Postgres y siguen andando.
//
// LO QUE EL SQL NO PUEDE HACER: los archivos del bucket `ip-docs` (constancias de IP y recetas). Borrar filas de
// storage.objects a mano deja los archivos huérfanos en el backend; van por Storage en el dashboard.
...
// Lo que la carga dejó, por protocolo. El script se frena si la base no da exactamente esto: es la única
// manera de saber que «tiene ivrs_code» sigue siendo «es de la carga» el día que se corra.
const INSCRIPCIONES_DE_LA_CARGA = { '222714': 8, ACT18301: 8, CKJX839D12302: 48, LTS17231: 8 }
const PERSONAS_DE_LA_CARGA = 64
const PROTOCOLOS_A_BORRAR = ['EFC18244', 'EFC18419', 'PROT-A', 'PROT-B']

// Se vacían enteras. El orden no importa: el bloque reintenta la que choca con una FK hasta que no queda
// ninguna (ver 3). Si una tabla no existe en la base, se saltea.
const TABLAS_A_VACIAR = [
  // Farmacia
  'dispensation_ip_documents', 'dispensation_items', 'stock_movements', 'dispensations',
  'dispensation_request_items', 'dispensation_habilitaciones', 'dispensation_requests',
  'ambulatory_dispensations', 'ip_units', 'reception_items', 'medication_lots', 'medication_receptions',
  'reposicion_pedidos', 'dispensation_daily_counters',
  // Coordinación, sobre visitas
  'visit_comments', 'visit_procedure_reports_ready', 'report_status_history', 'report_status',
  'alert_dismissals', 'visit_procedure_completions', 'visit_ip_closures', 'patient_timeline',
  'checklist_report_ready', 'checklist_completions', 'checklist_items', 'track_dispensations',
  'task_assignees', 'tasks', 'agenda_notes', 'protocol_alerts', 'feedback',
]

const NOTA_PROVISORIA = 'Fecha real provisoria: igual a la estimada. El listado del sitio no tiene registro de esta visita (carga del 2026-09-14).'

const lit = (s) => `'${String(s).replace(/'/g, "''")}'`
const arr = (xs) => `array[${xs.map(lit).join(', ')}]::text[]`

function molde(aplicar) {
  return `-- Spira · Limpieza de todo lo anterior a la carga de la prueba real — ${aplicar ? '2 · APLICAR' : '1 · SIMULAR (no guarda nada)'}
-- ============================================================================
...
declare
  ...
  v_esperado   jsonb := ${lit(JSON.stringify(INSCRIPCIONES_DE_LA_CARGA))}::jsonb;
  v_borrar     text[] := ${arr(PROTOCOLOS_A_BORRAR)};
  v_vaciar     text[] := ${arr(TABLAS_A_VACIAR)};
  v_estudios   text[];
  v_corte      timestamptz;
  v_tabla      text;
  v_pendientes text[];
...
begin
  ...
  if v_n <> ${PERSONAS_DE_LA_CARGA} then
    raise exception 'La carga tiene % personas y tenían que ser ${PERSONAS_DE_LA_CARGA}. No se borró nada.', v_n;
  end if;
  ...
  update public.patient_visits pv
     set arrived_at       = case when pv.real_date is not null then (pv.real_date + time '09:00') at time zone 'America/Argentina/Buenos_Aires' end,
         ...
         lleva_ip         = null,
         notes            = case when position(${lit(NOTA_PROVISORIA)} in coalesce(pv.notes, '')) > 0 then ${lit(NOTA_PROVISORIA)} end
   where pv.enrollment_id in (select enrollment_id from _carga)
     ...
     and (pv.left_at is not null or pv.attended_at is not null or pv.coordinator_id is not null or pv.coordinator_name is not null
          ...
          or pv.notes is distinct from case when position(${lit(NOTA_PROVISORIA)} in coalesce(pv.notes, '')) > 0 then ${lit(NOTA_PROVISORIA)} end);
  get diagnostics v_n = row_count;
  ...
  if v_n <> ${PERSONAS_DE_LA_CARGA} then
    raise exception 'Quedaron % pacientes y tenían que ser ${PERSONAS_DE_LA_CARGA}. No se borró nada.', v_n;
  end if;
...
for (const [nombre, sql] of [['1-simular.sql', molde(false)], ['2-aplicar.sql', molde(true)]]) {
  ...
  if (marcadores.length % 2 !== 0) {
    console.error(`✗ ${nombre}: ${marcadores.length} marcadores de dollar-quote (tiene que ser par).`)
    process.exit(1)
  }
  writeFileSync(resolve(aqui, nombre), sql)
}
console.log('✓ 1-simular.sql y 2-aplicar.sql generados.')
